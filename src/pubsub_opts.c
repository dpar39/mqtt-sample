...
void usage(struct PubSubOpts * opts, PubSubOptsNameValue * name_values, const char * program_name)
{
    ...
    printf("\n\n  -t (--topic)        : MQTT topic to %s to\n"
           "  -c (--connection)   : connection string, overrides host/port e.g wss://hostname:port/ws.  Use this "
           "option\n"
           ...
           opts->publisher ? "publish" : "subscribe",
           opts->host,
           opts->port,
           opts->publisher ? "publish" : "subscribe",
           opts->qos);
    ...
    printf("  --will-topic        : will topic on connect.  No default.\n"
           "  --will-payload      : will message.  If the will topic is set, but not payload, a null message will be "
           "set.\n"
        ...
        switch (MQTTProperty_getType(id)) {
            ...
            case MQTTPROPERTY_TYPE_UTF_8_ENCODED_STRING:
                printf("Property name %s value len %.*s\n",
                       name,
                       props->array[i].value.data.len,
                       props->array[i].value.data.data);
                break;
